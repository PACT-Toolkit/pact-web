...
// Route handler proxy for /api/pact/gateway/v1/rules → pact-gateway /v1/rules.
//
// In mock mode MSW intercepts the browser fetch before it reaches Next.js, so
// this handler is only hit in real mode. Translates the pact_session cookie →
// Bearer header that pact-gateway's authMiddleware expects, and propagates a
// rotated session back to the browser cookie (same pattern as
// app/api/pact/gateway/v1/check/route.ts).
...
async function proxy(req: NextRequest) {
  ...
  const upstream = await fetch(`${GATEWAY_URL}/v1/rules${req.nextUrl.search}`, {
    method: req.method,
    headers,
