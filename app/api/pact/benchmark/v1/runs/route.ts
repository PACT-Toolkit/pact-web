...
// Route handler proxy for /api/pact/benchmark/v1/runs → pact-gateway
// /v1/benchmark/runs.
//
// Benchmark run history is read through the gateway (auth-gated, rate-limited),
// not by hitting pact-benchmark's internal port directly. In mock mode MSW
// intercepts the browser fetch before it reaches Next.js, so this handler is
// only hit in real mode. Translates the pact_session cookie → Bearer header
// that pact-gateway's authMiddleware expects, and propagates a rotated session
