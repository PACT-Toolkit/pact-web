...
flowchart TB
    ...
    subgraph web["pact-web (Next.js 16 server, :3000)"]
        ...
        ProxyRoutes["Gateway proxy routes<br/>/api/pact/gateway/v1/*<br/>(proxyToGateway: cookie to Bearer)"]
...
`app/api/` and `app/v1/` hold route handlers, not pages: the auth gateway proxy (`app/api/auth/**`), the gateway proxy routes described above (`app/api/pact/**`), and the OAuth callback target (`app/v1/auth/callback/*`) that providers redirect back to directly.
...
- Every Orval group's `baseUrl` is the shared gateway prefix `/api/pact/gateway/v1`, which resolves to the proxy routes in [Architecture](#architecture). (PACT-923: `app/v1/{account,audit,files}` used to mirror pact-gateway's own path shape for callers hitting pact-web's `/v1/*` surface directly, but no Orval group and no other consumer ever called them, so they were removed.)
