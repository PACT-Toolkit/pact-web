...
test.describe('Test Lab', () => {
  ...
  test('has no accessibility violations', async ({ page }) => {
    ...
    // Pre-existing app-shell landmark issues, not introduced by PACT-465 --
    // same filter already applied in filter.spec.ts / gateway.spec.ts /
    // classifier.spec.ts / redactor.spec.ts / policy.spec.ts /
    // consensus.spec.ts. Remove once the shell is fixed.
