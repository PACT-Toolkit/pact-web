...
- **Don't put `'use client'` files under `framework/`.** Framework is the lowest layer (per the module hierarchy in `AGENTS.md`); client components belong in `src/components/` or feature `src/app/<feature>/ui/`.
